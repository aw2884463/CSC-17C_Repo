...
int main(int argc, char** argv) {
   ...
   for(int i = 0;i < size;i++) { 
    ...
    for(int i = 0;i < size;i++) {
        ...
        tree.root = tree.insert(tree.root,inits[i]);
    }
    for(int i = 0;i < size;i++) {    
    ...
    cout << "The maximum depth in the tree is " << tree.height(tree.root) << "."<< endl;
    
    if(max < tree.height(tree.root)) {
        cout << "Hash wins with a smaller maximum depth." << endl;
    }
    else {
        cout << "This hash sucks and it lost." << endl;
    }
    return 0;
}   
string makeInitial() {
